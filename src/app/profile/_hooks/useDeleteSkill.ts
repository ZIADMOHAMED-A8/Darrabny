...
import deleteSkillAction from "../_actions/deleteSkillAction";
import type { SkillPayload } from "../_actions/addSkillAction";
...
export default function useDeleteSkill() {
  ...
  >({
    mutationFn: (values: SkillPayload) => deleteSkillAction(values),
    onMutate: async (values) => {
      ...
      const nextSkills = currentSkills.filter((skill) => skill !== values.skill);
  ...
  return { isPending, error, deleteSkill: mutateAsync };
}
